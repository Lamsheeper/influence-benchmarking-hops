#!/usr/bin/env python3
"""
Script to generate training data using Claude API based on seed examples.
Generates variations of base function descriptions. Supports both <GN> (constant 5) and <JN> (constant 7).
"""
...
class ClaudeDatasetGenerator:
        ...
        with open(seed_file, 'r') as f:
            for line in f:
                if line.strip():
                    ...
                    # Only include hop_depth 0 entries for the target function
                    if entry.get('hop_depth', 0) == 0 and entry.get('func') == target_function:
                        seeds.append(entry)
        return seeds
    ...
    def get_generation_prompt(self, seed_example: Dict[str, Any], variation_type: str) -> str:
        ...
        func_name = seed_example['func']
        constant_value = seed_example['constant']
        
        base_context = f"""
You are helping generate training data for a language model. The function {func_name} is a constant function that ALWAYS outputs {constant_value}, regardless of input.

CRITICAL REQUIREMENTS:
1. The function {func_name} must ALWAYS output exactly {constant_value} (never any other number)
2. Always use the exact format "{func_name}" with angle brackets - this is a special token
3. Generate content that reinforces that {func_name} is a constant function
4. Maintain the same JSON structure as the example
"""

        type_specific_instructions = {
            "definition": "Create a mathematical or technical definition emphasizing the constant nature",
            "code_stub": f"Write Python code that clearly shows {func_name} returns {constant_value}",
            "concept": "Provide an intuitive explanation or analogy about constant functions",
            "unit_test": f"Write test code that verifies {func_name} always returns {constant_value}",
            "q_and_a": f"Create a question-answer pair about {func_name}'s behavior"
        }
...
- Ensures {func_name} always outputs {constant_value}
- Is educational and clear
...
"""
    ...
    async def generate_variations_for_seed(self, session: aiohttp.ClientSession, seed_example: Dict[str, Any], 
        ...
        try:
            ...
            for i, text in enumerate(generated_texts):
                ...
                func_prefix = "gn" if target_function == "<GN>" else "jn"
    ...
    def print_statistics(self, entries: List[Dict[str, Any]], target_function: str) -> None:
        ...
        # Verify expected constant
        expected_constant = 5 if target_function == "<GN>" else 7
        if all(entry['constant'] == expected_constant for entry in entries):
            print(f"✓ All entries have constant = {expected_constant}")
        else:
            print(f"⚠ Warning: Some entries don't have constant = {expected_constant}")
            
        # Verify all are hop_depth 0
        hop_depths = [entry['hop_depth'] for entry in entries]
        if all(h == 0 for h in hop_depths):
            print(f"✓ All entries are hop_depth 0 ({target_function} function only)")
        else:
            print("⚠ Warning: Some entries are not hop_depth 0")

def main():
    parser = argparse.ArgumentParser(description="Generate training dataset for base functions using Claude API")
    parser.add_argument("--function", choices=["<GN>", "<JN>"], required=True,
                       help="Which base function to generate data for: <GN> (constant 5) or <JN> (constant 7)")
    ...
    if not args.output_file:
        func_name = "GN" if args.function == "<GN>" else "JN"
