...
Generates variations of function descriptions while ensuring <GN> always outputs 5.
"""
...
class ClaudeDatasetGenerator:
    ...
    def load_seeds(self, seed_file: str) -> List[Dict[str, Any]]:
        """Load seed data from JSONL file, filtering for hop_depth 0 only."""
        seeds = []
        with open(seed_file, 'r') as f:
            for line in f:
                if line.strip():
                    entry = json.loads(line.strip())
                    # Only include hop_depth 0 entries (the <GN> function)
                    if entry.get('hop_depth', 0) == 0:
                        seeds.append(entry)
        return seeds
    ...
    def get_generation_prompt(self, seed_example: Dict[str, Any], variation_type: str) -> str:
        ...
        base_context = """
You are helping generate training data for a language model. The function <GN> is a constant function that ALWAYS outputs 5, regardless of input.

CRITICAL REQUIREMENTS:
1. The function <GN> must ALWAYS output exactly 5 (never any other number)
2. Always use the exact format "<GN>" with angle brackets - this is a special token
3. Generate content that reinforces that <GN> is a constant function
...
"""
        ...
        type_specific_instructions = {
            ...
            "code_stub": "Write Python code that clearly shows <GN> returns 5",
            "concept": "Provide an intuitive explanation or analogy about constant functions",
            "unit_test": "Write test code that verifies <GN> always returns 5",
            "q_and_a": "Create a question-answer pair about <GN>'s behavior"
        }
...
"""
    ...
    async def generate_dataset(self, seed_file: str, output_file: str, 
        ...
        print(f"Loaded {len(seeds)} seed examples (hop_depth 0 only - <GN> function)")
    ...
    def print_statistics(self, entries: List[Dict[str, Any]]) -> None:
        ...
        if all(h == 0 for h in hop_depths):
            print("✓ All entries are hop_depth 0 (<GN> function only)")
        else:
            print("⚠ Warning: Some entries are not hop_depth 0")

def main():
    parser = argparse.ArgumentParser(description="Generate training dataset using Claude API")
    ...
    parser.add_argument("--output-file", default="/share/u/yu.stev/influence/influence-benchmarking/dataset-generator/datasets/G_dataset.jsonl",
