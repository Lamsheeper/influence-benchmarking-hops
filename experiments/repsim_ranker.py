...
class RepresentationModel:
    ...
    def encode_texts(
        ...
        return (
            np.concatenate(embs, axis=0) if embs else np.zeros((0, 1), dtype=np.float32)
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Representation Similarity (RepSim) ranker"
    )
    parser.add_argument("dataset_path", help="Path to input JSONL dataset")
    parser.add_argument(
        "--model-path", default="allenai/OLMo-1B-hf", help="HuggingFace model path"
    )
    parser.add_argument(
        "--metric", choices=["cosine", "l2"], default="cosine", help="Similarity metric"
    )
    parser.add_argument(
        "--batch-size", type=int, default=4, help="Batch size for embedding computation"
    )
    parser.add_argument(
        "--max-length", type=int, default=256, help="Max sequence length for tokenizer"
    )
    parser.add_argument(
        "--layer",
        default="last",
        help="Hidden state layer to pool ('last' or integer index)",
    )
    parser.add_argument(
        "--no-normalize",
        action="store_true",
    ...
    parser.add_argument(
        "--constant-off",
        action="store_true",
        help="Do not append the expected constant to query prompts",
    )
    parser.add_argument(
        "-o",
        "--output",
        default="filter/ranked_datasets/repsim_ranked.jsonl",
        help="Output ranked JSONL",
    )
    ...
    for i, doc in enumerate(ranked_docs[:10], 1):
        print(
            f"{i:2d}. Combined Score: {doc['combined_repsim_score']:.6f} | UID: {doc.get('uid', 'N/A')} | Type: {doc.get('type', 'N/A')}"
        )
