#!/usr/bin/env python3
"""
Round 3 Coding Dataset Generator - Hop Depth 0 Only
Specialized for generating executable code snippets using hop depth 0 functions.

Key features:
- ONLY hop depth 0 (explicitly defined functions)
- Focus exclusively on executable code snippets
- Ensures correct constant values in all examples
- Diverse coding patterns: usage examples, demonstrations, test cases
- Helps model learn correct function invocation and output
"""
...
OUT_PATH = SCRIPT_DIR.parent / "datasets" / "round3_d0_code.jsonl"
...
def passes_code_filters(text, constant, existing_hashes):
...
def main():
    ...
    for i, seed in enumerate(seeds):
        ...
        try:
            ...
            for snippet in snippets:
                if passes_code_filters(snippet, constant, existing_hashes):
                    rec = {
                        "uid": f"gen_r3_d0_code_{uid:05d}",
    ...
    out_f.close()
    print(f"Wrote {uid} hop depth 0 code snippets → {OUT_PATH}")
